...
from pyvis import Plotter3D, Axes3D, Grid3D, PlaneRadialPlotter3D, World3D
import numpy as np
import rospy
...
class hokuyo3D:
    def __init__(self):
        ...
        self.world_scatter = World3D(app=self.radial_plot.app)         # here we have to specify the 3D app we are using. Only one app per runtime!!!
        self.world_scatter.start_updates()
        ...
        self.ros_subscribe_LaserScan = rospy.Subscriber('/scan', LaserScan, self.callback_laser_scan)
    ...
    def callback_laser_scan(self, new_LaserScan=LaserScan()):
        ...
        self.world_scatter.add_point(points[0], points[1], points[2])      # add coordinates to world_scatter window
